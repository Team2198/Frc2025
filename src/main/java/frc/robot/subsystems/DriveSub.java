...
public class DriveSub extends SubsystemBase {
 ...
 // PIDController pidController = new PIDController(0.007, 0, 0);
  ...
  public void periodic() {
    ...
    SmartDashboard.putNumber("xGoal", xGoal);
    SmartDashboard.putNumber("yGoal", yGoal);
  ...
  public boolean followPathGoal(){

    
    double xSpeed = xPidController.calculate(getPose().getX(), xGoal);
    double ySpeed = yPidController.calculate(getPose().getY(), yGoal);
    //double turningSpeed = turningPidController.calculate(getPose().getRotation().getDegrees(), rotation.getDegrees());

    robotRelative(xSpeed,ySpeed,0);

    return xPidController.atSetpoint() && yPidController.atSetpoint()&&turningPidController.atSetpoint();

  }

  public void updateGoals(double xAway, double yAway){
    xGoal = getPose().getX() + xAway; 
    yGoal = getPose().getY() + yAway; 
  ...
  public Command resetGoalCommand(){
    return this.runOnce(() -> resetGoals());
  }
